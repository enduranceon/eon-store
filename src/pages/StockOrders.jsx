...
export default function StockOrders() {
  ...
  const updateStatus = async (orderId, field, value) => {
    ...
    try {
      await StockOrder.update(orderId, { [field]: value });
    } catch (e) {
              ...
              {filtered.map(o => (
                <tr key={o.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => navigate(`/estoque/pedidos/${o.id}`)}>
                  ...
                  <td className="px-4 py-3 text-center">
                    <StatusSelect value={o.payment_status} options={PAYMENT_STATUS} onChange={v => updateStatus(o.id, 'payment_status', v)} />
                  </td>
                  <td className="px-4 py-3 text-center">
                    <StatusSelect value={o.delivery_status} options={DELIVERY_STATUS} onChange={v => updateStatus(o.id, 'delivery_status', v)} />
                  </td>
                </tr>
              ))}
            </tbody>
