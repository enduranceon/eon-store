...
import { ArrowLeft, User, Phone, Mail, Package, MessageCircle, Copy, Check, ExternalLink, Zap, QrCode, Link2, FileText, X } from 'lucide-react';
...
export default function StockOrderDetail() {
  ...
  const createAsaasCharge = async (billingType) => {
    ...
    // Salva CPF no pedido antes de criar cobrança
  ...
  const openWhatsAppDirect = () => {
    const phone = '55' + (order.customer_whatsapp || '').replace(/\D/g, '');
        ...
        <CardContent className="space-y-2 text-sm">
          ...
          {order.payment_method && (
            <p className="text-xs text-blue-600 font-medium mt-1">
              Preferência de pagamento: {PAYMENT_METHOD_LABEL[order.payment_method] || order.payment_method}
            </p>
          )}
        </CardContent>
      </Card>
