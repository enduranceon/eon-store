...
export default function StockOrderDetail({ orderId, embedded = false, onChanged }) {
  ...
  const createAsaasCharge = async (billingType) => {
    ...
    try {
      await createOrderCharge('stock', id, {
        billingType,
  ...
  const openWhatsApp = useCallback(() => {
    ...
    if (!hasNativeCharge && !order.external_payment_link) {
      openExternalCharge();
      return;
    }
  ...
  const saveExternalCharge = async () => {
    ...
    try {
      ...
      await saveOrderExternalCharge('stock', id, {
        externalLink: link,
  ...
  const ds = fulfillmentStatus('stock', order.delivery_status);
            ...
            <div className="space-y-4">
              {order.external_payment_link && (
                <ExternalChargeSummary
                  ...
                  onMessage={order.customer_whatsapp ? openWhatsApp : undefined}
              ...
              {(order.payment_preference || order.payment_method) && (
                <div className="flex items-center gap-2 text-sm bg-blue-50 border border-blue-200 rounded-xl px-4 py-2.5">
                  ...
                  <span className="font-semibold text-blue-900">{PAYMENT_METHOD_LABEL[order.payment_preference || order.payment_method] || order.payment_preference || order.payment_method}</span>
                </div>
              )}
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    { key: 'CREDIT_CARD', icon: <Zap className="w-4 h-4" />, label: (order.payment_preference || order.payment_method)?.startsWith('card_') ? `Cartão (${(order.payment_preference || order.payment_method).replace('card_', '')})` : 'Cartão' },
                  ].map(opt => (
              ...
              <Button variant="outline" className="w-full gap-2 border-amber-300 text-amber-700 hover:bg-amber-50" onClick={openExternalCharge}>
            ...
            <div>
              ...
              <Select value={deliveryStatus || LEGACY_DELIVERY_STATUS} onValueChange={handleDeliveryStatusChange}>
                ...
                <SelectContent>
                  ...
                  {fulfillmentStatusOptions('stock', order.delivery_status).map(status => (
                    <SelectItem key={status} value={status}>{fulfillmentStatus('stock', status).label}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <p className="mt-1 text-[11px] text-muted-foreground">{fulfillmentDefinition('stock').flowHint} Mostra a etapa atual e os próximos passos; pagamento é separado.</p>
              <p className="mt-1 text-[11px] text-muted-foreground">Fluxo: {fulfillmentFlowText('stock')}</p>
            </div>
            <div>
