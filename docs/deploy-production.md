...
Em pull requests, o workflow valida migrações somente no banco descartável e não escreve no Supabase remoto. Na `main`, ele faz o `dry-run` remoto e aplica todas as migrações pendentes que estiverem no Git antes de publicar as Edge Functions. Por isso, o plano remoto precisa ser conferido antes da integração e não pode conter uma pendência alheia ao release.

Desde 15/09/2026, `supabase functions deploy` (sem nome de função) publica **todas** as functions em `supabase/functions/`, não só a `api-v1` — inclusive as auxiliares como `generate-monthly-closing`, `fetch-asaas-receivables` e `fetch-contract-installments`. Qualquer alteração nessas functions passa a ir para produção junto com o merge na `main`, igual ao resto do código. Isso publica o *código*; não ativa a integração Asaas nem valida suas credenciais — isso continua dependendo de configuração de ambiente separada.
