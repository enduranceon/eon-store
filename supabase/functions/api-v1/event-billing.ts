...
function isEventRegistrationUrl(value: unknown): boolean {
...
function databaseError(error: { code?: string; message?: string }): Response {
  console.error("api-v1 event billing:", error);
  if (error.code === "P0002") {
  ...
  return jsonResponse({
    error: "Não foi possível atualizar a cobrança do evento",
    code: "database_error",
  }, 500);
}

export async function handleEventBillingRequest(
  req: Request,
  path: string,
  supabase: SupabaseClient,
  actorId: string,
): Promise<Response | null> {
  const match = path.match(/^\/orders\/event\/([^/]+)\/external-charge$/);
  if (!match) return null;
  const [, registrationId] = match;
  if (!UUID_PATTERN.test(registrationId)) {
    return jsonResponse(
      { error: "Inscrição inválida", code: "invalid_request" },
      400,
    );
  }
  ...
  if (req.method === "DELETE") {
    ...
    const { data, error } = await supabase.rpc("remove_event_external_charge", {
      p_order_id: registrationId,
  ...
  if (isEventRegistrationUrl(body.external_link)) {
    return jsonResponse({
      error:
        "Use o link da cobrança externa, não o link de inscrição do evento",
      code: "invalid_payment_link",
    }, 400);
  }

  if (
    !isHttpsUrl(body.external_link) ||
    !isCalendarDate(body.due_date) ||
  ...
  const { data, error } = await supabase.rpc("save_event_external_charge", {
    p_order_id: registrationId,
