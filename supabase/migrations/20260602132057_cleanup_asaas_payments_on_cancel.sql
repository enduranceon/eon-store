...
-- Função: marca linhas em asaas_payments como CANCELLED/REFUNDED
-- quando o pedido associado é cancelado ou reembolsado.
-- Recebe via TG_ARGV[0] o order_type ('presale', 'stock', 'contract').
CREATE OR REPLACE FUNCTION cleanup_asaas_payments_on_order_cancel()
RETURNS TRIGGER AS $$
DECLARE
  new_payment_status TEXT;
BEGIN
  -- Só age se o status mudou e o novo status é cancelled/refunded
  ...
  -- Atualiza linhas em asaas_payments que ainda estão "ativas"
...
-- Trigger para assessment_contracts
...
-- Trigger adicional para contratos: quando status (não payment_status) é cancelled
...
-- ────────────────────────────────────────────────────────────────────
-- BACKFILL: corrige as linhas órfãs já existentes no banco
-- ────────────────────────────────────────────────────────────────────

-- Presale cancelados/refunded
...
-- Contratos cancelados/refunded (payment_status OU status)
UPDATE asaas_payments ap
