...
CREATE OR REPLACE FUNCTION public.apply_stock_withdrawal(
...
BEGIN
  IF p_quantity IS NULL OR p_quantity <= 0 OR p_quantity > 10000000 THEN
    RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'Quantidade de retirada inválida';
  END IF;
  IF v_reason IS NULL THEN
    RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'Informe o motivo da retirada';
  END IF;
  IF length(v_reason) > 500 THEN
    RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'Motivo da retirada é muito longo';
  END IF;
  ...
  PERFORM set_config('eon.stock_movement_type', 'stock_withdrawal', TRUE);
  PERFORM set_config('eon.stock_movement_reason', v_reason, TRUE);
  ...
  IF jsonb_array_length(v_variations) > 0 THEN
    IF v_variation_name IS NULL THEN
      RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'Selecione o tamanho para registrar a retirada';
    END IF;
    ...
    IF v_current_quantity < p_quantity THEN
      RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'Saldo insuficiente para retirar essa quantidade do tamanho selecionado';
    END IF;
  ...
  ELSE
    ...
    IF COALESCE(v_product.quantity, 0) < p_quantity THEN
      RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'Saldo insuficiente para retirar essa quantidade';
    END IF;
...
REVOKE ALL ON FUNCTION public.apply_stock_withdrawal(UUID, INTEGER, TEXT, TEXT, UUID)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.apply_stock_withdrawal(UUID, INTEGER, TEXT, TEXT, UUID)
  TO service_role;

COMMENT ON FUNCTION public.apply_stock_withdrawal(UUID, INTEGER, TEXT, TEXT, UUID) IS
  'Removes stock atomically by product and optional variation, blocks negative balances, and preserves the inventory ledger context.';
