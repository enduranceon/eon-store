-- Permite que o valor de cada parcela do pagamento manual venha definido
-- pelo chamador (edge function), em vez de sempre dividir o total
-- igualmente entre as parcelas. Isso deixa o usuário ajustar cada parcela
-- (valor e data) na hora de registrar o pagamento, para bater com o que
-- realmente caiu no banco (ex: parcelas do Asaas não são sempre iguais).
--
-- A soma das parcelas ainda precisa bater com o total da venda -- isso
-- continua validado aqui, como última linha de defesa, mesmo que a edge
-- function já valide a mesma coisa antes de chamar esta função.
...
BEGIN
  ...
  IF abs(v_allocated_value - round(p_total, 2)) > 0.01 THEN
